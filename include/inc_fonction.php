...
//Liste déroulante des stagiairess
function OPTION_stagiaires(int $selected_id)
{
    global $link;
    $res = mysqli_query($link, "select * from stagiaires order by av_const,av_modele");
    $data = mysqli_fetch_all($res, MYSQLI_ASSOC);
    foreach ($data as $ligne) {
        extract(array_map("hsc", $ligne));
        $sel = ($selected_id == $sta_id) ? " selected " : "";
        echo "<option value='$sta_id' $sel>$av_const - $av_modele</option>";
    }
}
...
function OPTION_promotion(int $selected_id)
{
    global $link;
    $res = mysqli_query($link, "select * from promotion order by pro_id");
    $data = mysqli_fetch_all($res, MYSQLI_ASSOC);
    foreach ($data as $ligne) {
        extract(array_map("hsc", $ligne));
        $sel = ($selected_id == $pro_id) ? " selected " : "";
        echo "<option value='$pro_id' $sel>$pro_label</option>";
    }
}